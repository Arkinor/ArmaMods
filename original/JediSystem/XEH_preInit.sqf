...
fnc_Speedfocer_Act = {
    ...
    if (player getVariable "speedofforce" == false) then {
        ...
        [player, 10] remoteExec ["setAnimSpeedCoef", 0];
...
fnc_Vehicle_jedi_card = {
    ...
    hint format ["Техника %1 готова", getText(configFile >> "CfgVehicles" >> _veh >> "displayName")];
};
...
fnc_JediClone = {
    ...
    if (player getVariable "IMS_LaF_ForceMana" > 0.15)then{
        ...
        if (!(isNil {_unit getVariable "KG_CLONE"})) exitWith {};
        ...
        _unit setVariable ["KG_CLONE", 1,true];
        player spawn {
            _i = 0; 
    ...
    _unit setVariable ["KG_CLONE", nil,true];
            };
        };
  };
...
Particle_blesk_fnc = {
    ...
    // if (isNull _p || !alive _p) exitWith { hint "Цель недоступна"; };
    ...
    _electro = "#particlesource" createVehicle _ppos;   
...
Particle_blesk_fnc_2 = {
    ...
    _zone2 = "#particlesource" createVehicle _ppos;   
...
Jedi_fnc_ioniz = {
     ...
     if (player getVariable "IMS_LaF_ForceMana" > 0.1)then{
        ...
        // Скрываем объект от всех игроков
        ...
        [_bomb] remoteExec ["Particle_blesk_fnc_2", 0];
        [_bomb] remoteExec ["Particle_blesk_fnc", 0];
        _bomb setDamage 1;  
    
        // Определяем радиус поиска в 25 метров 
        _searchRadius = 25; 
        ...
        if (count _targets == 0) then { 
            systemChat "Нет ближайшего живого персонажа."; 
        }; 
        sleep 1;
        ...
        [_targets] spawn {
            params ["_targets"];
            {
                if (alive _x && ((name _x find "B1" != -1) or (name _x find "В1" != -1) or (name _x find "Дройд" != -1) or (name _x find "дройд" != -1) or (name _x find "droid" != -1) or (name _x find "Droid" != -1) or (name _x find "B2" != -1) or (name _x find "B1-" != -1) or (name _x find "BX" != -1) or (name _x find "OOM" != -1) or (name _x find "OR-" != -1) or (name _x find "BD-" != -1) )) then {
                        // sleep(random[0.1,0.2,1]); 
                        _x setDamage 1;
                }
            } forEach _targets;
        };
         ...
         // Теперь ищем технику в радиусе
        private _vehicles = nearestObjects [_creationPos, ["Car", "Tank", "Air", "HeliH", "Plane"], _searchRadius];

        if (count _vehicles == 0) then { 
            systemChat "Нет ближайшей живой техники."; 
        }; 

        // Проверяем найденные транспортные средства
        {
            if (alive _x) then {
                [_x, ["hithull", 0.5, true, _unit]] remoteExec ["setHitPointDamage", 0];
                [_x, ["hitturret", 1, true, _unit]] remoteExec ["setHitPointDamage", 0];
                [_x, ["hitgun", 1, true, _unit]] remoteExec ["setHitPointDamage", 0];
                [_x, ["hitengine", 1, true, _unit]] remoteExec ["setHitPointDamage", 0];
            }
        } forEach _vehicles;
        };
    };
...
Jedi_fnc_Fire = {
    ...
    [_unit] remoteExec ["createFireballdps", 0];
    };
...
Jedi_fnc_DeathTelekinez = {
    ...
    {
    private _size = (_x # 1);
    private _effectSize = [0, _size, 0];
    private _effect = "#particlesource" createVehicleLocal [_creationPos#0, _creationPos#1, 0];
    _effect setparticleparams [_x # 0, "", "SpaceObject", 1, 10, [0, 0, 3], [0, 0, 0], 0.1, 1.275+0.001, 1, 0, _effectSize, [[1, 1, 1, 1]], [1], 0.1, 0.05, "", "", "", 1, false, 0, []];
    _effect setParticleRandom [2, [30, 30, 3], [0, 0, 0], 0.2, _size / 2, [0, 0, 0, 0], 2, 0, 10, 0];
    _effect setParticleCircle [30, [0, 0*random [0.25, 0.5, 0.75], 0.2]];
    _effect setDropInterval 0.3;
    _effect_arr pushBack _effect;
    } forEach [
    ...
    {
    _unit = _this;
    _effect = "#particlesource" createVehicleLocal (position _unit);
...
Jedi_fnc_IH_Run = {
    ...
    if (_unit getVariable "ihscript" == true) then {
        ...
        // Запуск основного скрипта
            ...
            [_unit, 1.3] remoteExec ["setAnimSpeedCoef", 0];
